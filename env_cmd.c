...
void opt_env_i(char ***tmp_env, char **str, int *j)
{
	...
	free_str(*tmp_env);
	*tmp_env = NULL;
	if (tmp + 1 < size_str(str) && ft_strchr(str[tmp + 1],'=') && str[tmp + 1][0] != '=')
	{
		(*j)++;
...
int opt_u(char **str, int *i, t_env_tools env, char**tmp_env)
{
	if (str[*i + 1])
	{
		unset(&tmp_env, str[*i + 1], ft_strlen(str[*i + 1]), str);
		(*i)++;
	}
	else
	{
...
int opt_env(char **str, t_env_tools *env, char *path, int index_size[2])
{
	char **tmp_env;
	char **paths;
	...
	{
		if (opt_u(str, &index_size[0], *env, env->tmp_env))
			return (1) ;
	}
	else if (ft_strchr(str[index_size[0]],'=') && str[index_size[0]][0] != '=')
	{
		index_size[1] = (int)(ft_strchr(str[index_size[0]],'=') - str[index_size[0]]);
